...
class Source_Encoder(nn.Module):
    def __init__(self, input_nc, ngf=64, max_ngf=512, n_downsampling=2, norm_layer=nn.BatchNorm2d):
        ...
        layers = [
            nn.ReflectionPad2d((7 - 1) // 2),
            nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),
            norm_layer(ngf),
            activation,
            CBAM(ngf)  # Adding CBAM after the first convolution
        ]

        for i in range(n_downsampling):
            mult = 2**i
            layers += [
                nn.Conv2d(min(ngf * mult, max_ngf), min(ngf * mult * 2, max_ngf), kernel_size=3, stride=2, padding=1, bias=use_bias),
                norm_layer(min(ngf * mult * 2, max_ngf)),
                activation,
                CBAM(min(ngf * mult * 2, max_ngf))  # Adding CBAM after each downsampling layer
            ]
...
class Channel_Encoder(nn.Module):
    def __init__(self, ngf=64, max_ngf=512, C_channel=16, n_downsampling=2, norm_layer=nn.BatchNorm2d):
        ...
        self.res1 = nn.Sequential(
            ResnetBlock(min(ngf * mult, max_ngf), padding_type='zero', norm_layer=norm_layer, use_dropout=False, use_bias=use_bias),
            CBAM(min(ngf * mult, max_ngf))  # Adding CBAM after the first residual block
        )
        self.res2 = nn.Sequential(
            ResnetBlock(min(ngf * mult, max_ngf), padding_type='zero', norm_layer=norm_layer, use_dropout=False, use_bias=use_bias),
            CBAM(min(ngf * mult, max_ngf))  # Adding CBAM after the second residual block
        )
...
class Generator_dyna(nn.Module):
    def __init__(self, output_nc, ngf=64, max_ngf=512, C_channel=16, n_blocks=2, n_downsampling=2,
        ...
        # CBAM after the mask convolution
        self.cbam_mask_conv = CBAM(ngf_dim)

        model = []

        self.res1 = ResnetBlock(ngf_dim, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False,
                                use_bias=use_bias)
        self.cbam_res1 = CBAM(ngf_dim)  # CBAM after the first residual block

        self.res2 = ResnetBlock(ngf_dim, padding_type=padding_type, norm_layer=norm_layer, use_dropout=False,
                                use_bias=use_bias)
        self.cbam_res2 = CBAM(ngf_dim)  # CBAM after the second residual block
        ...
        for i in range(n_downsampling):
            ...
            model += [
                nn.ConvTranspose2d(min(ngf * mult, max_ngf), min(ngf * mult // 2, max_ngf),
                                   kernel_size=3, stride=2,
                                   padding=1, output_padding=1,
                                   bias=use_bias),
                norm_layer(min(ngf * mult // 2, max_ngf)),
                activation,
                CBAM(min(ngf * mult // 2, max_ngf))  # CBAM after each transpose layer
            ]

        model += [
            nn.ReflectionPad2d((5 - 1) // 2),
            nn.Conv2d(ngf, output_nc, kernel_size=5, padding=0)
        ]
        model += [nn.Sigmoid()]
    ...
    def forward(self, input, SNR):
        ...
        z = self.cbam_mask_conv(z)  # Applying CBAM after mask convolution
        z = self.mod1(self.res1(z), SNR)
        z = self.cbam_res1(z)  # Applying CBAM after first residual block
        z = self.mod2(self.res2(z), SNR)
        z = self.cbam_res2(z)  # Applying CBAM after second residual block
